...
var BugDispatch = {
    options: {
        ...
        if ("transform" in document.documentElement.style) this.transform = this.transforms.w3c; else for (var b = ["Moz", "webkit", "O", "ms", "Khtml"], c = 0, c = 0; c < b.length; c++)if (b[c] + "Transform" in document.documentElement.style) {
    ...
    add_events_to_bug: function (a) {
        ...
        if (a.alive) {
            ...
            "random" === b && (b = this.modes[this.random(0, this.modes.length -
                1, !0)]);
            if ("fly" === b) a.stop(), a.flyRand(); else if ("nothing" !== b)if ("flyoff" === b) a.stop(), a.flyOff(); else if ("die" === b) a.die(); else if ("multiply" === b && !this.multiplyDelay && this.bugs.length < this.options.maxBugs) {
                var c = SpawnBug(), b = JSON.parse(JSON.stringify(this.options)), d = a.getPos(), e = this;
...
var Bug = {
    options: {wingsOpen: !1, walkSpeed: 2, flySpeed: 40, edge_resistance: 50, zoom: 10}, initialize: function (a, b) {
        ...
        if (this.animating && this.alive && this.active) {
            ...
            "_lastTimestamp" in this || (this._lastTimestamp =
                a);
            ...
            if (!(40 > c || (200 < c && (c = 200), this._lastTimestamp = a, 0 >= --this.toggle_stationary_counter && this.toggleStationary(), this.stationary))) {
                if (0 >= --this.edge_test_counter && this.bug_near_window_edge() && (this.angle_deg %= 360, 0 > this.angle_deg && (this.angle_deg += 360), 15 < Math.abs(this.directions[this.near_edge] - this.angle_deg))) {
                    ...
                    this.large_turn_angle_deg = Math.abs(a) < Math.abs(d) ? a :
                        d;
                    this.edge_test_counter = 10;
                ...
                if (0 >= --this.small_turn_counter) this.angle_deg += this.random(1, this.options.maxSmallTurnDeg), this.next_small_turn(); else {
        ...
        if (!this.bug && this.active) {
            var a = this.wingsOpen ? "0" : "-" + this.options.bugHeight + "px", b = document.createElement("div");
            b.className = "bug";
        ...
        this.bug.style.backgroundPosition =
            this.stationary ? "0 " + a : "-" + this.options.bugWidth + "px " + a
    }, walkFrame: function () {
        ...
        var b = this.bug.top, c = this.bug.left, d = c - a.left, e = b - a.top, f = Math.atan(e / d);
        50 > Math.abs(d) + Math.abs(e) && (this.bug.style.backgroundPosition = -2 * this.options.bugWidth + "px -" + 2 * this.options.bugHeight + "px");
        30 > Math.abs(d) + Math.abs(e) && (this.bug.style.backgroundPosition = -1 * this.options.bugWidth + "px -" + 2 * this.options.bugHeight + "px");
        if (10 > Math.abs(d) + Math.abs(e)) this.bug.style.backgroundPosition = "0 0", this.stop(), this.go(); else {
            var g = Math.cos(f) * this.options.flySpeed, f = Math.sin(f) * this.options.flySpeed;
        ...
        var b = this.bug.top, c = this.bug.left, d = a.left - c, e = a.top - b;
        ...
        if (this.bug) {
            this.stop();
            var a = Math.round(4 * Math.random() - .5), b = document, c = b.documentElement,
                d = b.getElementsByTagName("body")[0], b = window.innerWidth || c.clientWidth || d.clientWidth,
    ...
    walkIn: function () {
        ...
        this.stop();
        var a = this.random(0, 3), b = {}, c = document, d = c.documentElement, e = c.getElementsByTagName("body")[0],
        ...
        var b = this.bug.top, c = document, d = c.documentElement, c = c.getElementsByTagName("body")[0],
            e = window.innerHeight || d.clientHeight || c.clientHeight, e = e - this.options.bugHeight,
        ...
        if (0 === a || 1 === a) c.twitchTimer = setTimeout(function () {
            ...
            c.twitchTimer = setTimeout(function () {
                b++;
                c.bug.style.backgroundPosition = "-" + (2 *
                    a + b % 2) * c.options.bugWidth + "px 100%";
                c.twitch(a, ++b)
            }, c.random(300, 800))
        }, this.random(1E3, 1E4))
    }, rad2deg: function (a) {
